...
<body>
    <nav class="navbar navbar-expand-lg navbar-light bg-light" >
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
          <ul class="navbar-nav mr-auto">
            <li class="nav-item active">
                <form method="get" action ="{{ route('adminmsgs') }}">
                    <button type="submit" > 
                        Message
                    </button></form>
            </li>
       ...
       <table style="width:90% ;margin-left:5% ; margin-right:5%;margin-top:5%">
        ...
        <tr><td><img src="{{$item->picture_url}}" width="150px" height="150px"/></td>
        <td><form method="get" action ="{{ route('api:approvepic') }}">
            <input type="hidden" name="id" value="{{$item->id}}">
            <button class="login100-form-btn" type="submit" style="background-color:rgb(81, 168, 81);"> 
                APPROVE
            </button></form></td>
            <td><form method="get" action ="{{ route('api:declinepic') }}">
                <input type="hidden" name="id" value="{{$item->id}}">
